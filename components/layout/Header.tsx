  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "border-b border-black/[0.06] bg-white/80 backdrop-blur-lg dark:border-white/[0.06] dark:bg-[#0a0a0a]/80"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8 h-16">
        ...
        <a
          ...
          className="font-heading text-lg font-bold text-zinc-900 dark:text-zinc-100 tracking-tight hover:text-accent transition-colors"
        >
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-zinc-600 dark:text-zinc-400 rounded-lg
                         transition-all duration-200 hover:text-zinc-900 dark:hover:text-zinc-100
        ...
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden overflow-hidden border-t border-black/[0.06] dark:border-white/[0.06]
                       bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-lg"
          >
            <nav className="flex flex-col gap-1 px-4 py-4">
              {NAV_LINKS.map((link) => (
                <button
                  key={link.href}
                  onClick={() => handleNavClick(link.href)}
                  className="text-left px-4 py-3 text-sm font-medium text-zinc-600 dark:text-zinc-400 rounded-lg
